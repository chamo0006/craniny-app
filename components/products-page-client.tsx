...
export function ProductsPageClient({ initialProducts, categories, initialColors }: ProductsPageClientProps) {
  ...
  // Cuenta cuántos filtros están activos en tiempo real
  const activeFilterCount = useMemo(() => {
    let n = selectedColors.size + selectedSizes.size
    if (selectedCategory) n++
    if (maxPrice < computedMaxPrice) n++
    return n
  }, [selectedCategory, selectedColors, selectedSizes, maxPrice, computedMaxPrice])
  ...
  const renderFilterContent = () => (
    <div className="space-y-6">
      ...
      {/* Color — flex-wrap + punto de color real */}
      <div>
